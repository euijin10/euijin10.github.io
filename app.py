...
def load_movies() -> pd.DataFrame:
    ...
    """
    movies = pd.read_csv("./data/movies_imdb.csv")  # 파일 경로 확인
    return movies
...
def prepare_query_favourites() -> dict:
    ...
    data = get_movies()
    st.markdown(
        "어떤 영화를 볼지 고민 중이신가요? **최애 영화 알려주시면** 취향에 맞게 추천해 드릴게요"
    )
...
def prepare_query_rating() -> dict:
    ...
    st.markdown(
        "어떤 영화를 봐야 할지 모르겠나요? 다음 무작위로 선택된 10편의 영화입니다. **별점**을 매겨주시면 그 별점에 따라 추천해 드리겠습니다."
    )
    query = {}
...
def display_movie(movie_id: int) -> None:
    ...
    with col1:
        if validators.url(str(movie["cover_url"].iloc[0])):
            ...
            st.image("path/to/default/image.jpg")  # 기본 이미지 경로 설정

    with col2:
        if not pd.isnull(movie["title"].iloc[0]) and not pd.isnull(movie["year"].iloc[0]):
        ...
        if not pd.isnull(movie["director"].iloc[0]):
            st.markdown(f"**Director(s):** {', '.join(movie['director'].iloc[0].split('|'))}")
        if not pd.isnull(movie["cast"].iloc[0]):
            st.markdown(f"**Cast:** {', '.join(movie['cast'].iloc[0].split('|')[:10])}, ...")
...
# Header image
st.image("data/cover_collage.jpg")  # 경로 확인
